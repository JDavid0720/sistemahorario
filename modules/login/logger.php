...
if (!empty($_POST['txtuser']) and !empty($_POST['txtpass'])) {
    ...
    if ($result = $conexion->query($sql)) {
        if ($row = mysqli_fetch_array($result)) {
            ...
            if (!empty($_POST['remember_session'])) {
                ...
                header('Location: /sistemaHorario/home');
            } else {
                $_SESSION['user'] = $user;
                $_SESSION['name'] = $name;
            ...
            echo '
					...
					<input type="text" class="text" name="txtuser" placeholder="Ingrese CI / Pass" autofocus required />
					<input type="password" class="textcontrasena" name="txtpass" placeholder="Ingrese la ontraseña" autocomplete="off" required />
					<div class="forgot-pass">
                        <a class="un" href="./modules/login/backup.php">¿Olvidaste la contraseña?</a>
                    </div>
					...
					<button class="button" type="submit">Inicio de Sesion</button>
                ';
        }
    }
} else {
    echo '
			...
			<input type="text" class="text" name="txtuser" placeholder="Ingrese CI / Pass" autofocus required />
			<input type="password" class="textcontrasena" name="txtpass" placeholder="Ingrese la contraseña" autocomplete="off" required />
            <div class="forgot-pass">
                <a class="un" href="./modules/login/backup.php">¿Olvidaste la contraseña?</a>
            </div>           
            <div class="pretty p-svg p-curve p-smooth">
