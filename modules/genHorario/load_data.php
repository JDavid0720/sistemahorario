...
function horaactividad($t, $d,$act)
{
    include '../conexion.php';

$sql = "SELECT COUNT(*) AS count
	FROM horario 
	WHERE horario.Id_actividad='$act' AND horario.Id_tiempo = '$t' AND horario.Id_dia = '$d'";
...
function ejecutar($ci)
{
    ...
    for ($j = 0; $j < $totalAsignaturas; $j++) {
        ...
        if ($_SESSION["CI"][$j] === $ci) {
            ...
            $HG=$_SESSION["HG"][$j];
            $HI=$_SESSION["HI"][$j];
            $HVHPP=$_SESSION["HVHPP"][$j];
			$carreraM=$_SESSION["Carrera"][$j];
			$ciclo=$_SESSION["Id_Ciclo"][$j];
            
            if ($HI > 0 )  {
				
                generarextra($act, $HI, "EXTRA", "HI", $ci);
			}
    ...
    for ($j = 0; $j < $totalAsignaturas; $j++) {
        ...
        if ($_SESSION["CI"][$j] === $ci) {
            ...
            if ($OAD> 0 )  {
                ...
                generarextra($act, $OAD, "EXTRA", "OAD", $ci);
			}
    ...
    for ($j = 0; $j < $totalAsignaturas; $j++) {
        ...
        if ($_SESSION["CI"][$j] === $ci) {
            ...
            if ($HG > 0 )  {
                ...
                generarextra($act, $HG, "EXTRA", "HG", $ci);
			}
    ...
    for ($j = 0; $j < $totalAsignaturas; $j++) {
        ...
        if ($_SESSION["CI"][$j] === $ci) {
            ...
            if ($HVHPP > 0 )  {
                ...
                generarextra($act, $HVHPP, "EXTRA", "HVHPP", $ci);
			}
...
function generarHorarios($act, $cantidad, $jornada, $tipo, $ci, $carr, $cicl)
{
    ...
    duplicadocurso($num, $dia, $carr, $cicl) > 0 ||horaactividad($num, $dia,$act)|| $num == 0)) {
    ...
    horaactividad($num, $dia,$act)||
    horaactividad($num1, $dia,$act)||
	$num == 0)) {
        $num = mt_rand($minh, $maxh);
        if($tipo=='HVIR')
		{
...
function generarextra($act, $cantidad, $jornada, $tipo, $ci)
{
    ...
    // Búsqueda anticipada para evitar duplicados
    $attempts = 0;
    while (($attempts <$itr) && (duplicado($num, $dia, $ci) > 0 || horaactividad($num, $dia,$act) || $num == 0)) {
        if($tipo=='HI')
        {
            $num = mt_rand(7, 10);
        }
    ...
    $attempts = 0;
    while (($attempts <$itr) && (duplicado($num, $dia, $ci) > 0 ||
	duplicado($num1, $dia, $ci) > 0 || horaactividad($num, $dia,$act)>0 ||
    horaactividad($num1, $dia,$act)>0|| 
	$num == 0)) {
        if($tipo=='HI')
        {
            $num = mt_rand(7, 10);
    ...
    // Búsqueda anticipada para evitar duplicados
    $attempts = 0;
    while (($attempts <$itr) && (duplicado($num, $dia, $ci) > 0 || horaactividad($num, $dia,$act)>0|| 
     $num == 0)) {
        if($tipo=='HI')
        {
            $num = mt_rand(7, 10);
    ...
    $attempts = 0;
    while (($attempts <$itr) && (duplicado($num, $dia, $ci) > 0 ||
	duplicado($num1, $dia, $ci) > 0 ||horaactividad($num, $dia,$act)>0||horaactividad($num1, $dia,$act)>0|| $num == 0)) {
        if($tipo=='HI')
        {
            $num = mt_rand(7, 10);
        }
    ...
    $attempts = 0;
    while (($attempts <$itr) && (duplicado($num, $dia, $ci) > 0 ||
	duplicado($num1, $dia, $ci) > 0 || horaactividad($num, $dia,$act)>0||horaactividad($num1, $dia,$act)>0||
	$num == 0)) {
        if($tipo=='HI')
        {
            $num = mt_rand(7, 10);
